...
>
	{#snippet children({ thumbItems })}
		...
		{#each thumbItems as thumb (thumb.index)}
			<!-- aria-label lands on the Root above too, but Root isn't
			     role="slider" — only the thumb is, so an accessible name
			     given to this component has to reach here explicitly or a
			     screen reader announces nothing but a bare number. -->
