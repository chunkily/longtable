...
3. **Buttons that act are a verb or two.** `Join`, not `Join room` — the card it sits in already
   said what is being joined. Note that a link _out_ of a dead-end screen was rewritten the other
...
| Rooms aren't listed anywhere, so the code is the only way in.           | _(deleted)_                                       | A comment in a `Card.Description`                                    |
...
over-fitted and you should write the plain version. When it disagrees with the _examples_ — when
...
## After renaming any string, grep the specs

```bash
grep -rn "the old text" web/e2e
```

Playwright finds things by their visible text — `getByRole('button', { name: … })`,
`getByText(…)`, `getByLabel(…)` — so a reworded string is a broken locator, and nothing else in
the build will tell you. Types pass, lint passes, the app runs.

This is not hypothetical: `join-flow.spec.ts` sat broken across two commits because
`No room with that code` became `Room not found!` and `Back to the start` became
`Click here to go back home`. Both were one-line wording changes, which is exactly the kind of
edit nobody runs the suite for.

Watch for **substring** matches, which is where this gets subtle rather than obvious. Playwright
matches accessible names by substring unless `exact: true`, so shortening a label can silently
change _which_ element a locator resolves to instead of failing outright. `Join room` → `Join` made
`{ name: 'Join' }` also match `Join a room` on another step. Renaming a label to a _prefix_ of
itself is the dangerous shape.
