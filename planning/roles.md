...
room code, so there is no state between "hasn't heard of this room" and "is in it" worth naming.

A Host is not a GM. They run the server and needn't be at any table on it, which is why the
recovery path for a GM who has lost a room code or a password runs through them
