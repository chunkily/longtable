...
- [ ] I can override the theme (light / dark / system) from the [options page](../backlog/options-page.md); my choice is persisted to client storage and wins over `prefers-color-scheme` from then on, on that device
- [ ] The canvas/stage background (the area outside the map image) also switches, not just surrounding UI
- [ ] The map image itself, uploaded token art, and the existing light/dark drawing-stroke palettes (see [dark-map-stroke-palette](../backlog/dark-map-stroke-palette.md)) are unaffected — this is app chrome, not map content
